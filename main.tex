...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % 中文封面内容
  \title{基于单目视觉的\\同时定位与地图构建方法\\}%一般情况下扉页和封皮、书脊共用一个标题文本，可以不用定义\spinetitle（仅硕博有用）, \covertitle（本硕博均有用）和\encovertitle（仅本科有用）。特殊情况见下。
...
\mainmatter
  ...
  \include{chapter/chap-VSLAM}
  \include{chapter/chap-PTAM}
  \include{chapter/chap-ORBSLAM}
  \include{chapter/chap-DSLAM}
